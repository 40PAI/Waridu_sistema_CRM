-- Migration: 0052_create_rpc_move_event_functions.sql
-- Description: Add RPC functions for moving events in the pipeline (Kanban drag-and-drop), with overloads and JSON wrapper for compatibility.

-- Add pipeline_rank column if it doesn't exist
DO $$
BEGIN
...
-- Helper function to rebalance ranks in a column
...
-- Overload for UUID (if events.id is uuid)
...
-- Overload for BIGINT (if events.id is integer/bigint)
CREATE OR REPLACE FUNCTION public.rpc_move_event(
  p_event_id bigint,
  p_new_status text,
...
-- JSON wrapper for robustness (handles type conversion internally)
