  ...
  const exportToPDF = async (data: ExportData) => {
    try {
      ...
      const metrics = [
        ...
        { label: "Margem (%)", value: data.consolidated.revenue > 0 ? `${((data.consolidated.profit / data.consolidated.revenue) * 100).toFixed(1)}%` : "0%" }
      ];
      ...
      // Tabela de rentabilidade por evento (máximo 15 eventos)
      ...
      // Adicionar nota se há mais eventos
      if (data.events.length > 15) {
        tableData.push(["...", "...", "...", "...", "...", `+${data.events.length - 15} eventos`]);
      }
      ...
      doc.autoTable({
        ...
        didDrawPage: (data) => {
          ...
          doc.text(
            `Página ${data.pageNumber} de ${pageCount} | Relatório Financeiro Automático`, 
            margin, 
      ...
      throw new Error(error instanceof Error ? error.message : "Falha ao gerar PDF");
    }
  };
