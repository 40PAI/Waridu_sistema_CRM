  ...
  const calculateEventCosts = React.useCallback((event: Event) => {
    ...
    if (event.roster?.teamMembers) {
      ...
      event.roster.teamMembers.forEach((member: any) => {
        ...
        const categoryId = employee?.technicianCategoryId; // Fixed: Use technicianCategoryId directly
    ...
    <div className="space-y-6">
      ...
      {alerts.length > 0 && (
        <Card className="border-yellow-200 bg-yellow-50 dark:bg-yellow-900/20 dark:border-yellow-700">
          ...
          </Card<dyad-write path="src/pages/finance/Reports.tsx" description="Fix DateRange type mismatch">
import * as React from "react";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
