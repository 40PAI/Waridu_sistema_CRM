...
export default function NewProjectPage() {
  ...
  const [creatingClientOpen, setCreatingClientOpen] = React.useState(false); // placeholder if you implement modal
  ...
  React.useEffect(() => {
    ...
    const fetchPhases = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        if (error) {
          console.warn("Failed to load pipeline phases:", error);
  ...
  const handleSubmit = async (data: ProjectFormData) => {
    ...
    try {
      ...
      // navigate back to pipeline passing created id so pipeline page can scroll
      navigate("/crm/pipeline", { state: { createdProjectId: result.id } });
    } catch (err: any) {
  ...
  return (
    <div className="p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Preencha os dados para criar um novo projeto no pipeline.</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
                      ...
                      <FormControl>
                        <div className="flex gap-2">
                          <Combobox
                            ...
                            className="flex-1"
                          />
                          <Button type="button" variant="outline" onClick={() => setCreatingClientOpen(true)}>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <Button variant="outline" onClick={() => navigateBackToPipeline(navigate)}>Cancelar</Button>
      ...
      {/* placeholder for create client modal - not implemented inline here */}
      </div>
  );
}

/**
 * helper navigate back to pipeline without creating a project
 */
function navigateBackToPipeline(navigate: any) {
