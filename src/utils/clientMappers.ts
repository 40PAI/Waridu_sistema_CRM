...
export function formToEventsInsert(input: NewProjectForm): Database.EventsInsert {
  ...
  if (input.startTime !== undefined && input.startTime !== '') {
    dbPayload.start_time = `${input.startTime}:00`; // Map startTime → start_time (HH:mm:ss format)
  }
  if (input.endTime !== undefined && input.endTime !== '') {
    dbPayload.end_time = `${input.endTime}:00`; // Map endTime → end_time (HH:mm:ss format)
  }
