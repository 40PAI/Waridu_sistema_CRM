  ...
  const handleOAuthLogin = async (provider: 'google' | 'microsoft') => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/`, // Redireciona para home após login
        },
      });
      if (error) throw error;
    } catch (error: any) {
      showError(`Erro ao fazer login com ${provider}. Tente novamente.`);
    }
  };
          ...
          <div className="space-y-2">
            ...
            >
              <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24">
                ...
                <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21<dyad-write path="src/components/auth/LoginForm.tsx" description="Completa a atualização do LoginForm com botões OAuth.">
"use client";
